    ...
    try:
        ...
        if not financials.empty and len(financials.columns) >= 2:
            ...
            if (
                ...
                try:
                    price_hist_1y = ticker.history(period="1y")
                    if not price_hist_1y.empty and previous_eps:
                        previous_price = price_hist_1y["Close"].iloc[0]
        ...
        try:
            ...
            logger.warning(f"{ticker_symbol} テクニカル取得失敗: {e}")
